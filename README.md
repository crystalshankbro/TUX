...
1. Download `tux` from [github.com/crystalshankbro/TUX](https://github.com/crystalshankbro/TUX)
2. Move the downloaded `tux` file to your home folder (`~/`)
3. Run in terminal:
...
After that just type `tux` in any terminal from anywhere.

---

## Uninstall

If you installed Tux via `self-install` or moved it to `/usr/local/bin`, run:

```bash
sudo rm /usr/local/bin/tux
```
...
Opens the interactive menu. **Clears the terminal** and shows the menu with arrow key navigation.
...
| Command | Description |
|---------|-------------|
| `tux` | Open interactive menu (clears terminal) |
| `tux ru` | Open menu in Russian |
| `tux en` | Open menu in English |
...
1. Скачай `tux` с [github.com/crystalshankbro/TUX](https://github.com/crystalshankbro/TUX)
2. Перенеси скачанный файл `tux` в домашнюю папку (`~/`)
...
После этого просто пиши `tux` в любом терминале из любого места.

---

## Удаление

Если ты установил Tux через `self-install` или переместил его в `/usr/local/bin`, выполни:
...
Открывает интерактивное меню. **Очищает терминал** и показывает меню с навигацией стрелками.
...
| Команда | Описание |
|---------|----------|
| `tux` | Открыть интерактивное меню (очищает терминал) |
| `tux ru` | Открыть меню на русском |
| `tux en` | Open menu in English |
